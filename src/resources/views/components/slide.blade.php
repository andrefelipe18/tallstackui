...
<div x-cloak
    @if ($wire) x-data="tallstackui_slide(@entangle($entangle), @js($configurations['overflow'] ?? false))"
     @else
         x-data="tallstackui_slide(false, @js($configurations['overflow'] ?? false))" @endif
    x-show="show" @if (!$configurations['persistent']) x-on:keydown.escape.window="show = false;" @endif
    x-on:slide:{{ $open }}.window="show = true;" x-on:slide:{{ $close }}.window="show = false;"
    @class(['relative', $configurations['zIndex']]) {!! $events !!}>
    <div x-show="show" x-transition:enter="ease-in-out duration-300" x-transition:enter-start="opacity-0"
        ...
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @class([
            ...
            $personalize[
                'blur.' .
                    ($configurations['blur'] === true ? 'sm' : $configurations['blur'])
            ] ?? null => $configurations['blur'],
        ])></div>
    <div @class($personalize['wrapper.second'])>
        <div @class($personalize['wrapper.third'])>
            <div @class([
                ...
                <div x-show="show" x-transition:enter="transform transition ease-in-out duration-700"
...
x-transition:leave-end="@if ($configurations['left']) -translate-x-full @elseif ($configurations['top']) -translate-y-full @elseif ($configurations['bottom']) translate-y-full @else translate-x-full @endif"
                    ...
                    <div @class([
                        ...
                        <div @class($personalize['header'])>
                            <div @class([
                                'flex items-start',
                                'justify-between' => $title !== null,
                                'justify-end' => $title === null,
                            ])>
                                @if ($title)
                                    <h2
                                        @if ($title instanceof \Illuminate\View\ComponentSlot) {{ $title->attributes->class($personalize['title.text']) }}
                                        @else
                                            @class($personalize['title.text']) @endif>
                                        {{ $title }}</h2>
                                @endif
                                <button type="button" x-on:click="show = false">
                                    <x-dynamic-component :component="TallStackUi::component('icon')" :icon="TallStackUi::icon('x-mark')"
                        ...
                        @if ($footer)
                            <div
                                @if ($footer instanceof \Illuminate\View\ComponentSlot) {{ $footer->attributes->class([
                                    $personalize['footer'],
